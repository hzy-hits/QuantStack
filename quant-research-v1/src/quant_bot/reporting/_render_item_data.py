...
def _price_momentum_table(item: dict) -> list[str]:
    ...
    if mom:
        ...
        lines += [
            ...
            f"| Log Return 20D | {_fmt_pct(mom.get('log_return_20d_pct'), 2)} |",
            f"| Denoised Log Slope 10D | {_fmt_pct(mom.get('denoised_log_slope_10d_pct'), 3)}/day |",
            f"| Vol-Normalized Log Return | {_fmt_val(mom.get('log_return_vol_norm_20d'), 2)}\u03c3 |",
            f"| FFT Trend/Noise | {_fmt_val(mom.get('fft_signal_to_noise'), 2)} |",
            f"| Haar Noise Energy | {_fmt_pct((mom.get('haar_noise_energy') * 100.0) if mom.get('haar_noise_energy') is not None else None, 1)} |",
