...
def rank_and_select(
    ...
    if not selection_policy:
        ...
        event_extras = [
            ...
            if s["sub_scores"]["event"] > 0.5 and s["symbol"] not in top_syms
        ][:5]
        for extra in event_extras:
            ...
            if not replaceable:
                break
            worst_idx, _ = min(replaceable, key=lambda x: x[1]["score"])
            top[worst_idx] = extra
    ...
    core_items = sorted(
        ...
        key=lambda x: (x.get("report_score", x["score"]), x["score"]),
        reverse=True,
    )
    event_tape_items = sorted(
        [x for x in scored if x.get("report_bucket") == "event_tape"],
        key=lambda x: (x["score"], x.get("report_score", x["score"])),
        reverse=True,
    )
