...
- 宏观/regime 输出服务组合层 beta hedge：必须保留大盘风险偏好、宽度、资金和跨市场压力，供 long alpha + beta hedge 判断使用；不得把宏观叙事升级成单票 Fresh Entry
- A股没有单一 VIX 等价物；payload 的 A股恐惧/贪婪是内部风险偏好代理，必须保留该口径
- 数据缺失写 `[缺失]`，不分析缺失原因
- 不给交易建议，不做叙事
- 禁用词：综合考量、谨慎乐观、值得关注、密切跟踪、不确定性较大、用法、操作建议、请注意、这只是
- "## 判断" 三句话不要写成程序化模板。每句都要有数字 + 因果关系；不要使用 "需密切跟踪" "建议关注" 这类报告腔结尾
- 宏观数据必须标注参考期和滞后月数
