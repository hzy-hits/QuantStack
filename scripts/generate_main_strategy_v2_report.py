...
def build_market_regime_score(us_db: Path, as_of: date) -> dict[str, Any]:
    ...
    N = len(rows)
    # Need 5d history for derivative
    if N < 6:
    ...
    dpc1 = pc[i] - pc[i-1]
    dpc5 = pc[i] - pc[i-5]
    # Statistics for normalization (use whole window)
    import math
...
# build_serenity_crosscheck + render_serenity_crosscheck_section extracted to
# scripts/sections/serenity.py (Phase B.0 of REFACTOR_PLAN.md)
