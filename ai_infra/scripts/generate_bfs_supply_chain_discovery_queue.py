...
DEFAULT_NAME_ZH_OVERRIDES = Path("data/company_name_zh_overrides.csv")
...
def ticker_parts(ticker: str) -> list[str]:
    return [
        part.strip().upper()
        for part in re.split(r"\s*/\s*|\s+", ticker or "")
        if part.strip()
    ]


def is_china_or_hk_row(row: dict[str, str]) -> bool:
    market_text = " ".join([row.get("market_country", ""), row.get("asset_pool", "")])
    ticker_text = row.get("ticker", "").upper()
    return bool(
        any(marker in market_text for marker in ["A股", "中国资产池", "H股", "港股", "China", "Hong Kong"])
        or re.search(r"\.(SZ|SH|BJ|HK)\b", ticker_text)
    )


def company_zh_for(row: dict[str, str], overrides: dict[str, str]) -> str:
    company = row.get("company", "").strip()
    if not is_china_or_hk_row(row):
        return company
    for ticker in ticker_parts(row.get("ticker", "")):
...
def build_task(row: dict[str, str], index: int, name_zh_overrides: dict[str, str]) -> dict[str, str]:
    theme = classify_theme(row)
    keywords = EXTRACTION_KEYWORDS["default"] + EXTRACTION_KEYWORDS.get(theme, [])
    ticker = row.get("ticker", "")
    company = row.get("company", "")
    company_zh = company_zh_for(row, name_zh_overrides)
    ...
    return {
        ...
        "agent_prompt": (
            f"Seed: {ticker} {company} / {company_zh}. Module: {module}. Read primary filings/product pages and extract named "
...
def write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    fields = [
        "task_id", "priority", "theme", "region_bucket", "seed_ticker", "seed_company", "seed_company_zh",
...
def write_report(path: Path, tasks: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "",
        markdown_table(
            ["task_id", "priority", "theme", "region", "seed", "中文名", "depth", "module"],
            [[
                t["task_id"], t["priority"], t["theme"], t["region_bucket"],
                f'{t["seed_ticker"]} {t["seed_company"]}', t["seed_company_zh"], t["bfs_depth"], t["module"][:70],
            ] for t in top],
        ),
        "",
        ...
        "",
        "1. `new_candidate`: company/ticker/company_zh/exchange/country.",
...
def compact_seed_lines(tasks: list[dict[str, str]], max_rows: int) -> list[str]:
    ...
    return [
        ...
        f"{t['seed_company_zh']} | {t['theme']} | {t['module']} | {t['current_pool']}"
        for t in selected
    ]
...
- 新候选公司清单：ticker、company_zh、exchange/country、BFS depth、dependency edge、为什么相关、需要核验的原文、主要反证；
...
def main() -> None:
    ...
    name_zh_overrides = load_name_zh_overrides(args.name_zh_overrides)
    tasks = [build_task(row, index + 1, name_zh_overrides) for index, row in enumerate(rows)]
