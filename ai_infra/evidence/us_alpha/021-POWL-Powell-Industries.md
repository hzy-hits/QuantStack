# POWL Powell Industries US alpha evidence card

状态：draft evidence card, pending original-source verification
边界：研究线索，不是投资建议、买卖建议、目标价或仓位建议。

## 基本信息

| 字段 | 内容 |
| --- | --- |
| ticker | POWL |
| company | Powell Industries |
| BFS depth | D3-D4 |
| module | Electrical switchgear |
| cluster | power_thermal |
| current pool | 候选池 |
| total score | 76 |

## 为什么挖

AI data centers are constrained by power delivery and heat removal; this bucket tests time-to-power bottlenecks.

## 第一轮要回答的问题

How much backlog/revenue is data-center related? Are lead times tight? Does gross margin improve with AI demand?

## 必找原文

10-K/10-Q; earnings release; backlog/order disclosures; investor presentation; product pages.

## 原文证据

| 指标 | 原文位置 | 原文已证明什么 | 不能证明什么 | 备注 |
| --- | --- | --- | --- | --- |
| Revenue / segment revenue |  |  |  |  |
| AI/datacenter mix |  |  |  |  |
| Orders / backlog / customer demand |  |  |  |  |
| Gross margin / operating margin |  |  |  |  |
| Customer concentration |  |  |  |  |
| CapEx / inventory / cash flow |  |  |  |  |

## 升级 / 降级条件

| 方向 | 条件 |
| --- | --- |
| 升级 | Backlog, book-to-bill, data center mix, lead time, and margin all improve from AI/DC demand. |
| 降级 | Orders are pulled forward, data centers slip, cooling/power products commoditize, or customer pricing pressure rises. |

## 当前反证

周期订单、客户集中、扩产后瓶颈缓解

## 结论分层

| 层级 | 内容 |
| --- | --- |
| 原文已证明 |  |
| 合理推论 |  |
| 待原文核验 | 待原文核验: DC customer and backlog |
| 主要反证 | 周期订单、客户集中、扩产后瓶颈缓解 |
| 当前动作 | 先补原文，不做组合动作 |
