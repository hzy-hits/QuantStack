...
- `scripts/generate_main_strategy_v2_report.py:474` 的 `us_alpha_factory_sleeve_id()` 只认 LOW/core/executable/trending。
...
- `scripts/generate_main_strategy_v2_report.py:2972` 当前只做当日 risk overlay。
...
- `scripts/generate_main_strategy_v2_report.py` 继续能生成相同的当前日 portfolio overlay。
- 新模块不改变任何现有 sleeve 的 money_status、role、metrics。

Hedge 逻辑不能新写第二份。当前 `generate_main_strategy_v2_report.py` 已有 `_select_beta_hedge` / portfolio overlay 相关逻辑；只有在 historical hedge ledger 落地时才抽到 `scripts/lib/hedge.py`，再由当前日 overlay 和历史 ledger 共用，避免两个 hedge 实现漂移。
...
- `scripts/generate_main_strategy_v2_report.py:455` 仍按单票 LOW/core/trending 做 V2 policy，这是美股主题票被卡住的主要代码入口。
...
- `scripts/generate_main_strategy_v2_report.py:summarize_cn()` 增加 current-day tape candidate emitter。
...
- `scripts/generate_main_strategy_v2_report.py:us_alpha_factory_sleeve_id()` 增加 theme membership 判断。
...
- `scripts/generate_main_strategy_v2_report.py:summarize_us()` 输出 theme_id、basket score、theme hedge benchmark、theme concentration risk。
...
- 从 `scripts/generate_main_strategy_v2_report.py` 抽取当前 `_select_beta_hedge`、beta floor、hedge ratio、benchmark selection 到 `scripts/lib/hedge.py`。
...
- `scripts/generate_main_strategy_v2_report.py` 读取历史 hedged evidence，把当前日 R 计划和历史净 alpha 证据连起来。
...
- `generate_main_strategy_v2_report.py` must fail fast at startup if a current production R path lacks a promoted sleeve contract.
...
```bash
python3 -m py_compile scripts/run_alpha_sleeve_backtest.py scripts/generate_main_strategy_v2_report.py
...
python3 scripts/generate_main_strategy_v2_report.py --start 2026-03-01
```
...
8. 只有新 sleeve 的 hedged LCB 过线并写入 `promoted_sleeves`，再改 `generate_main_strategy_v2_report.py` 和 CN/US ranker 给生产 R。
